...
4. Check if there are changes in `css/` directory compared to the latest tag:
   - Get the latest tag using `git describe --tags --abbrev=0`
   - Run `git diff <latest-tag> HEAD -- css/` to check for changes
   - If there are no changes (diff is empty), ask the user whether to continue with the release or abort
5. Update version in the following files:
...
6. Stage the changes and create a commit with message:
   ```
   Release vX.Y.Z
...
8. Create a git tag named `vX.Y.Z`
